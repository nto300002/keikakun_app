...
## 個別支援計画の正規化設計とパフォーマンス工夫

### 正規化の方針

個別支援計画ドメインは、以下の3テーブルに分割して正規化している。

```
support_plan_cycles（サイクル親テーブル）
  └── support_plan_statuses（ステップ進捗）
  └── plan_deliverables（成果物ファイル）
```
...
| 観点 | 理由 |
|---|---|
| 更新単位の独立 | サイクル全体の情報（開始日・更新期限）と、各ステップの進捗（完了日・完了者）は更新タイミングと担当者が異なる |
| 行数の爆発防止 | 1サイクルに5ステップ × 複数ファイルを1行に収めると NULL が増えスキーマが複雑化 |
| 責任追跡 | `support_plan_statuses.completed_by` でステップごとの完了者を記録するためにステップ粒度でのレコードが必要 |

#### `office_id` の非正規化（意図的）

`support_plan_cycles` は `welfare_recipient_id` → `office_welfare_recipients` → `office_id` の経路でも事業所を辿れるが、直接 `office_id` カラムを持たせている。

**理由**:
- テナント分離フィルタ `WHERE office_id = ?` は全クエリで必ず実行されるため、JOIN を省いてインデックス直引きにする
- 受給者が事業所を移籍した場合でも「計画を作成した時点の事業所」を正確に記録できる

```python
# 非正規化の効果: JOIN なしでインデックス検索
stmt = select(SupportPlanCycle).where(
    SupportPlanCycle.office_id == office_id,        # ← 直接フィルタ（index hit）
    SupportPlanCycle.welfare_recipient_id == recipient_id
)
```

#### `is_latest_cycle` フラグの役割

最新サイクルを取得する際、全サイクルを `ORDER BY cycle_number DESC LIMIT 1` でスキャンせず、`is_latest_cycle = True` のレコードを直引きできる。

```python
# サイクル更新時の排他制御
# 旧サイクルの is_latest_cycle を False にしてから新サイクルを is_latest_cycle=True で作成
old_cycle.is_latest_cycle = False
new_cycle = SupportPlanCycle(..., is_latest_cycle=True)
```

新サイクル作成時は必ず旧サイクルの `is_latest_cycle` を `False` に更新してから INSERT することで、常に1レコードのみ `True` になるよう Service 層で管理している。

---

### インデックス設計（個別支援計画ドメイン）

#### `support_plan_cycles` のインデックス
...
class SupportPlanCycle(Base):
    ...
    __table_args__ = (
        # テナント分離 + 受給者絞り込みの複合インデックス（最頻出クエリ）
        Index('ix_support_plan_cycles_office_recipient',
              'office_id', 'welfare_recipient_id'),

        # 期限アラートバッチ: office_id + next_renewal_deadline の範囲検索
        Index('ix_support_plan_cycles_office_deadline',
              'office_id', 'next_renewal_deadline'),

        # 最新サイクル取得: is_latest_cycle=True のみを対象とした部分インデックス
        Index('ix_support_plan_cycles_latest',
              'welfare_recipient_id', 'is_latest_cycle',
...
| インデックス名 | カラム | 用途 |
|---|---|---|
| `ix_support_plan_cycles_office_recipient` | `(office_id, welfare_recipient_id)` | 受給者一覧ページの計画サイクル取得（テナント分離必須） |
| `ix_support_plan_cycles_office_deadline` | `(office_id, next_renewal_deadline)` | 期限アラートバッチの範囲検索（`deadline < today + N days`） |
| `ix_support_plan_cycles_latest` | `(welfare_recipient_id, is_latest_cycle)` | 最新サイクル直引き。`is_latest_cycle=True` 行のみが対象の部分インデックスでインデックスサイズを削減 |

**部分インデックスを使う理由**: 全サイクルのうち `is_latest_cycle=True` は常に1件のみ。全件インデックスを持つより1/N のサイズで高速検索が可能。

---

#### `support_plan_statuses` のインデックス
...
class SupportPlanStatus(Base):
    ...
    __table_args__ = (
        # サイクル詳細ページ: 1サイクルの全ステップを取得
        Index('ix_support_plan_statuses_cycle', 'plan_cycle_id'),

        # 期限アラートバッチ: 未完了ステップの絞り込み
        Index('ix_support_plan_statuses_cycle_completed',
              'plan_cycle_id', 'completed'),

        # 特定ステップ種別の進捗確認（monitoring ステップ期限チェック等）
        Index('ix_support_plan_statuses_office_step',
              'office_id', 'step_type', 'completed'),
    )
```

| インデックス名 | カラム | 用途 |
|---|---|---|
| `ix_support_plan_statuses_cycle` | `plan_cycle_id` | サイクル詳細表示時の `selectinload(SupportPlanCycle.statuses)` の裏側 |
| `ix_support_plan_statuses_cycle_completed` | `(plan_cycle_id, completed)` | 未完了ステップのみを絞り込む期限管理クエリ |
| `ix_support_plan_statuses_office_step` | `(office_id, step_type, completed)` | 事業所全体での特定ステップ（例: `monitoring` 期限切れ）の一括チェック |

**`completed` を複合インデックスに含める理由**: 期限アラートバッチは「未完了かつ期限が近い」ステップのみを対象にする。`completed=False` 行は全ステータス行の大半を占めるため単純な部分インデックスより効果が低いが、`plan_cycle_id + completed` の組み合わせで結果セットが大幅に絞れる。

---

#### `plan_deliverables` のインデックス
...
class PlanDeliverable(Base):
    ...
    __table_args__ = (
        # サイクルの成果物一覧取得（selectinload の裏側）
        Index('ix_plan_deliverables_cycle', 'plan_cycle_id'),

        # 事業所全体の成果物管理画面: office_id + deliverable_type での絞り込み
        Index('ix_plan_deliverables_office_type',
              'office_id', 'deliverable_type'),
    )
```

---

### クエリパターン別のインデックス効果まとめ

| クエリパターン | 使用インデックス | 効果 |
|---|---|---|
| 受給者詳細ページ（サイクル一覧） | `ix_support_plan_cycles_office_recipient` | office_id + recipient_id で即座に絞り込み |
| 最新サイクルの取得 | `ix_support_plan_cycles_latest`（部分） | `is_latest_cycle=TRUE` 行のみの小さい索引 |
| ダッシュボード期限アラート | `ix_support_plan_cycles_office_deadline` | `next_renewal_deadline < today + 30` の範囲スキャン |
| サイクル詳細（ステップ進捗） | `ix_support_plan_statuses_cycle` | `selectinload` のサブクエリが index scan |
| 未完了ステップの期限バッチ | `ix_support_plan_statuses_cycle_completed` | `completed=False` を plan_cycle_id 単位で高速取得 |
| 成果物PDF一覧 | `ix_plan_deliverables_cycle` | `joinedload` → `selectinload` 混在クエリのボトルネック解消 |

**一般原則**: `office_id` を先頭カラムに置く複合インデックスを基本とし、テナント分離フィルタを常に index の第1キーとして活用する。単一カラムのインデックスは `office_id` による絞り込み後にのみ有効なため、複合インデックスで1クエリあたりのスキャン行数を最小化する。
